...
uint8_t avr_timer8_value(struct timer8_regs *regs) { return regs->tcnt; }

uint16_t avr_timer16_value(struct timer16_regs *regs) { return regs->tcnt; }

static uint8_t avr_timer_no(uintptr_t regs)
{
    switch ((uintptr_t)regs)
    {
    case (uintptr_t)&TCCR0A:
        return 0;
    case (uintptr_t)&TCCR1A:
        return 1;
    case (uintptr_t)&TCCR2A:
        return 2;
#ifdef TCCR3A
    case (uintptr_t)&TCCR3A:
        return 3;
#endif
#ifdef TCCR4A
    case (uintptr_t)&TCCR4A:
        return 4;
#endif
    default:
        return 0;
...
{
    struct timsk_array *TIMSK = (struct timsk_array *)&TIMSK0;
    bits_assign(TIMSK->timsk[avr_timer_no((uintptr_t)regs)], 0x1, en);
}
...
/*void tc_start(uint8_t channel, uint32_t divider) {
    ...
    if (bits_mask(1 << channel, TIMERS_8BIT_MAP)) {
        struct timer_8bit_regs* timer = (struct timer_8bit_regs*)
timers_array[channel]; bits_mask_assign(timer->tccr_b, 7, divider);
    }

    else if (bits_mask(1 << channel, TIMERS_16BIT_MAP)) {
        struct timer_16bit_regs* timer = (struct timer_16bit_regs*)
timers_array[channel]; bits_mask_assign(timer->tccr_b, 7, divider);
    }
}
...
void tc_ovf_int_enable(uint8_t channel, void(*handler)(void*)) {
    ...
    if (bits_mask(1 << channel, TIMERS_8BIT_MAP)) {
        struct timer_8bit_regs* timer = (struct timer_8bit_regs*)
timers_array[channel]; TIMSK[channel] |= 0x01;
    }
