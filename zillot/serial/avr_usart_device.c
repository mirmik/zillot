...
{
	avr_usart_device_s * dev = mcast_out(priv, avr_usart_device_s, u);
...
void avr_usart_device_rx_irq_handler(void * arg)
{
	...
	if (usart->u.handler) usart->u.handler(usart->u.handarg, UART_IRQCODE_RX);
}

void avr_usart_device_dre_irq_handler(void * arg)
{
	struct avr_usart_device_s* usart = (struct avr_usart_device_s*) arg;
	if (usart->u.handler) usart->u.handler(usart->u.handarg, UART_IRQCODE_TX);
}
...
{
	struct avr_usart_device_s* usart = (struct avr_usart_device_s*) arg;
	if (usart->u.handler) usart->u.handler(usart->u.handarg, UART_IRQCODE_TC);
}
...
int avr_usart_device_setup(uart_s * priv, int32_t baud,
                           char parity,
                           uint8_t databits,
                           uint8_t stopbits)
{
